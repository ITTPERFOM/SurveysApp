...
public class SurveyActivity extends AppCompatActivity  implements form.OnFragmentInteractionListener {
    ...
    public void Next(View view){
        int ErrorQuestion = 0;
        findViewById(R.id.FragmentReport).setVisibility(View.INVISIBLE);
        try
        {
            lm = (LinearLayout) findViewById(R.id.activity_survey);
            Validations validations= new Validations();
    ...
    private void CreateListenerImage(int controlid) {
        Button button=  (Button) findViewById(controlid+idKey2);
        button.setOnClickListener( new ButtonClickHandler( button) );
    }

    public void onPhotoTaken(Button button)
    {
    ...
    {
        Button button;
        public ButtonClickHandler(Button button) {
            this.button = button;
        }
